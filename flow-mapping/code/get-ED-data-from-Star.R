...
clean_room_names <- function(x) {
  ...
  x = gsub(" CHAIR","",x)
  x = gsub("MAJ-CHAIR","MAJORS",x)
  x = gsub("^UTC [A-z]+ ROOM","UTC",x)
  x = gsub(" $","",x)
  return(x)
}
