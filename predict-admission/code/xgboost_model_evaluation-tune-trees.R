...
# an elaborate way to find out all the scores for these models
...
outFile <- paste0("EDcrowding/predict-admission/data-output/xgb_metrics_demo-arrchars-locdurations_60-mins_tune-trees-8_all-scores_",today(),".rda")
save(scores, file = outFile)

save_chart("demog-admchar-mins_tune-trees-8_some-scores",
           scores %>% 
             filter(.metric %in% c("accuracy", "kap", "precision", "recall")) %>% 
             ggplot(aes(x = trees, y = .estimate, color = .metric)) +
             geom_point(size = 3) + geom_line() +
             theme_classic() +
             scale_x_continuous(breaks = as.numeric(tree_grid$trees)) +
             labs(title = "Tuning for trees with 60 min model with 5 fold validation", 
...
outFile <- paste0("EDcrowding/predict-admission/data-output/xgb_pred_demo-arrchars-locdurations_60-mins_tune-trees-8_best-kap_",today(),".rda")
save(pred, file = outFile)
