...
for (i in (1:length(time_pts))) {
  ...
  df = merge(in_ED, preds_all_ts, by = c("csn", "timeslice"))
  ...
  distr = bind_cols(sample_time = time_pts[i], num_adm_pred = num_adm, probs = pgf)
...
distr_coll[, cdf:= cumsum(probs), by = sample_time]
...
for (i in (1:length(time_pts))) {
  ...
  in_ED = summ[presentation_time < time_pts[i] &
                 last_ED_discharge > time_pts[i], .(csn, presentation_time, adm, elapsed = difftime(time_pts[i], presentation_time, units = "mins"))]
 
  adm = sum(in_ED$adm %in% c("direct_adm", "direct_adm"))
...
for (i in 1:nrow(adm_coll)) {
  ...
  cutoff_cdf_at$date = adm$sample_time[i]
...
cutoff_cdf_normalised %>% pivot_longer(actual_less_than_lower_limit:model_lower_limits, 
  ...
  labs(title = paste0("Admission probability distribution evaluation - based on ", length(time_pts), " randomly sampled time points of interest"), 
       ...
       For 20 cutoff points on the cdf (shown on the X axis), the number of patients predicted by the model is calculated at this point",
       x = "X")  +
