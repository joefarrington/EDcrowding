...
for (i in 1:nrow(adm)) {
  ...
  for (j in 1:nrow(cutoff_cdf_at)) {
    ...
    cutoff_cdf_at$data_upper_limit_of_cdf[j] =  
...
for (i in (1:nrow(adm))) {
  data_point_cdf = distr_collect  %>% 
    filter(date == adm$date[i], num_adm == adm$fake_adm[i]) %>% select(date, num_adm, lower_limit_of_cdf = cdf)
  
  data_point_cdf$upper_limit_of_cdf = as.numeric(distr_collect  %>% 
...
# proportion of instances where lower_limit_of_cdf <= cut-off on cdf

cutoff_cdf_at = as_tibble(seq(0,1,.1)) %>% rename(cutoff = value)

for (j in 1:nrow(cutoff_cdf_at)) {
  cutoff_cdf_at$proportion_of_instance[j] = nrow(each_data_point_cdf %>% filter(lower_limit_of_cdf <= cutoff_cdf_at$cutoff[j]))
}
