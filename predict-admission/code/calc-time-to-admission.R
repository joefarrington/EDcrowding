...
permutations = unique(tta_prob[, .N, by = .(epoch, in_set, time_of_report, timeslice, tta_hr)][, .(epoch, in_set, time_of_report, timeslice)])
...
# there is a neater way of doing this from in_ED_at_time_pt_all 
...
for (i in 1:length(pre_post_time_pts)) {
  ...
  in_ED_at_time_pt = summ[first_ED_admission < pre_post_time_pts[i] &  first_outside_proper_admission > pre_post_time_pts[i] 
                          & adm %in% c("indirect_adm", "direct_adm"), 
                          .(csn, epoch, in_set, first_ED_admission = with_tz(first_ED_admission, tzone = "UTC"), 
                            first_outside_proper_admission = with_tz(first_outside_proper_admission, tzone = "UTC"))]
  
  in_ED_at_time_pt[, time_pt := with_tz(pre_post_time_pts[i], tzone = "UTC")]
...
in_ED_at_time_pt_all[, epoch := "Pre + Post"]
in_ED_at_time_pt_all[, in_set := "Train"]
...
# check
tta_prob[epoch == "Pre + Post" & in_set == "Train"] %>% ggplot(aes(x = tta_hr, y = cdf)) + geom_point() + facet_grid(time_of_report~timeslice)
...
tta_prob_val = tta_prob_old[epoch == "Post" & in_set == "Val"]
tta_prob_val[, epoch := "Val"]
tta_prob = bind_rows(tta_prob_old, tta_prob, tta_prob_val)
