...
sqlQuery <- "select hv.encounter as csn, lo.order_datetime, lo.request_datetime, lo.lab_battery_id, lb.battery_code
  from star.hospital_visit hv,
  star.lab_order lo,
  star.lab_battery lb
...
sqlQuery <- "  select hv.encounter as csn,  lr.abnormal_flag, lr.comment, lr.units, lr.range_high, lr.range_low, lr.result_last_modified_time, lr.value_as_real, lr.value_as_text, ltd.test_lab_code
    from star.hospital_visit hv,
      star.lab_result lr,
    star.lab_order lo,
...
save(lab_results_raw, file = paste0('EDcrowding/predict-admission/data-raw/lab_results_raw',today(),'.rda'))
