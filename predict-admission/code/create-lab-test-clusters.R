...
# Further investigation now that battery codes are available --------------
...
# get lab orders that are made in ED - lab_order_real has been filtered to include only orders made in ED
load("~/EDcrowding/predict-admission/data-raw/lab_orders_real_2021-04-14.rda")


# Create adjacency matrix 

# marge with lab_order_real to get dataset of lab results that relate to orders made while in ED
lab_results_raw_ED = merge(lab_results_raw, lab_orders_real, by = c("csn", "lab_order_id"))

l1 = lab_results_raw_ED[, .N, by = test_lab_code]

l2 = lab_results_raw_ED[, .N, by = .(battery_code, test_lab_code)]
...
# create an edgelist by counting the number of times each lab test co-occurs
# reducing it to 5
codes = unique(l1[N>200, test_lab_code])
...
ggdendrogram(hclust, rotate = FALSE, size = 2) + 
  labs(title = "Dendogram of co-occuring lab results used more than 200 times since April 2019") +
  geom_hline(aes(yintercept = 11.5, colour = "red")) +
  theme(legend.position = "none")
...
# forcing a cluster distribution of 44
dend <- color_branches(dend, k=44) #, groupLabels=iris_species)
...
par(mar = c(3,3,3,7))
plot(dend, 
     main = "Dendogram of co-occuring lab results used more than 200 times since April 2019, forced into 44 clusters", 
     horiz =  TRUE,  nodePar = list(cex = .007))
legend("topleft", legend = iris_species, fill = rainbow_hcl(3))
