...
if (tune_nr) {
  ...
  tune_ps = ParamSet$new(list(
    ...
    ParamInt$new("nrounds", lower = 1, upper = 250)
  ))
  ...
  for (ts_ in timeslices) {
    ...
    for (tuning_measure in c(msr("classif.logloss"), msr("classif.auc"), msr("classif.tp"),
                      msr("classif.fp"), msr("classif.tn"), msr("classif.fn"))) {
      
      if (tuning_measure$id == "classif.auc") {
        eval_metric = "auc"
      } else {
        eval_metric = "logloss" }
      ...
      instance = TuningInstanceSingleCrit$new(
        ...
        resampling = rsmp("cv"),
        measure = tuning_measure,
        search_space = tune_ps,
        terminator = trm("evals") # alternative is trm("stagnation", iters = 5, threshold = 1e-3)
      )
      ...
      res[, model := name_tsk]
      res[, tuning_measure := tuning_measure$id] # copy the parameter for this instance from the table header
      res[, score := res[,2]]
      res[,2] <- NULL
      res[, test := "nrounds"]
      res[, eval_metric := eval_metric]
      ...
      res$val_tp = pred_val$score(msr("classif.tp"))
      res$val_fp = pred_val$score(msr("classif.fp"))
      res$val_fn = pred_val$score(msr("classif.fn"))
      res$val_tn = pred_val$score(msr("classif.tn"))
      res$val_auc = pred_val$score(msr("classif.auc"))
      res$val_logloss = pred_val$score(msr("classif.logloss"))
      ...
      pred[, model := name_tsk]
      pred[, tuning_measure := tuning_measure$id]
      nr_pred <- bind_rows(nr_pred, pred)
      
      save(nr_pred, file = paste0("EDcrowding/predict-admission/data-output/nr_pred_",today(),".rda"))
...
nr_results[, .SD[which.max(score)], by = tuning_measure]

n = nr_pred[truth != response, .N, by = row_id]

all_madcap = data.table()

for (meas in unique(nr_pred$tuning_measure)) {
  mc <- as.data.table(madcap(nr_pred[tuning_measure == meas ]))
  mc[, tuning_measure := meas]
  all_madcap <- bind_rows(all_madcap, mc)
}
...
all_madcap %>% ggplot(aes(x))+ 
  ...
  facet_wrap(vars(tuning_measure), nrow = 1, ncol = 6, scales = "free")
