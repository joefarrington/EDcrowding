...
dm <- matrix_60 %>% 
  ...
  select(-mrn, -csn, -csn_old, -birthdate, -ED_duration_final,
        ...
        # -colnames(matrix_60)[grep("rass", colnames(matrix_60))],wwwwwerearefa
...
xgb_grid <- grid_latin_hypercube(
  ...
  finalize(mtry(), juice(dm_prepped)), # this explains why so mtry values were so high!! dm_train has more variables than my current model
  learn_rate(),
  size = 1
)
...
# set up workflow
xgb_wf <- workflow() %>%
  add_recipe(dm_recipe) %>%  # note - this doesn't include a recipe ; with a recipe you can use juiced() to see training data
  add_model(xgb_spec)

# set up parameters to try
# from https://juliasilge.com/blog/animal-crossing/
tree_grid <- grid_regular(trees(), levels = 3)
