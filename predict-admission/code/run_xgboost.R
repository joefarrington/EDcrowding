...
tree_depth_grid <- grid_regular(tree_depth(), levels = 8)
...
xgb_tuned_tree_depth <- tune_grid(
  object = xgb_wf, 
  resamples = dm_folds, 
  grid = tree_depth_grid, 
  metrics = metric_set(roc_auc, accuracy, kap, precision, recall, ppv, npv)
)


# Save results ------------------------------------------------------------

outFile = paste0("EDcrowding/predict-admission/data-output/xgb_results_demo-arrchars-locdurations-counts_60-mins_tune-tree-depth-8_",today(),".rda")
save(xgb_tuned_tree_depth, file = outFile)
