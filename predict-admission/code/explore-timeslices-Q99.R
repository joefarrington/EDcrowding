...
# Same file as explore-timeslices, but adds the 99th quartile to some of the charts
...
chart_data2 = summ_timeslice_long[adm %in% c("direct_adm", "indirect_adm") & value > 0, list(.N, 
                                                                                       as.numeric(quantile(ED_duration - timeslice, .25)),
                                                                                       as.numeric(median(ED_duration - timeslice)), 
                                                                                       as.numeric(quantile(ED_duration - timeslice, .75)),
                                                                                       as.numeric(quantile(ED_duration - timeslice, .99))), by = timeslice] 
setnames(chart_data2, "V2", "Q25")
setnames(chart_data2, "V3", "Median")
setnames(chart_data2, "V4", "Q75")
setnames(chart_data2, "V5", "Q99")

chart_data2 = chart_data2 %>% pivot_longer(N:Q99, names_to = "quartile")

chart_data2 %>% 
  mutate(quartile = factor(quartile, levels = c("Q25", "Median", "Q75", "Q99"))) %>% 
  filter(quartile != "N") %>% 
  ggplot(aes(x = as.factor(timeslice), y = value, col = quartile, group = quartile)) + geom_point() + geom_line() +
  theme(panel.grid.minor = element_blank()) + 
  labs(y = "Elapsed time to onward admission \nafter beginning of timeslice", 
...
setnames(chart_data_s, "V5", "Q99")

chart_data_s = chart_data_s %>% pivot_longer(N:Q99, names_to = "quartile")
...
chart_data_s %>% 
  ...
  labs(y = "Elapsed time to onward admission \nafter beginning of timeslice", 
       x = "Timeslice",
       title = "Time to admission after earliest cut point of timeslice: randomly sampled timeslices - admitted patients only") +
  theme(legend.position = "bottom")
...
chart_data_s2 = summ_s[adm %in% c("direct_adm", "indirect_adm") & ED_duration < 24*60, list(.N, 
                                                                                           ...
                                                                                           as.numeric(quantile(ED_duration - time_since_sample_dttm, .75)),
                                                                                           as.numeric(quantile(ED_duration - time_since_sample_dttm, .99))), by = timeslice] 
setnames(chart_data_s2, "V2", "Q25")
setnames(chart_data_s2, "V3", "Median")
setnames(chart_data_s2, "V4", "Q75")
setnames(chart_data_s2, "V5", "Q99")

chart_data_s2 = chart_data_s2 %>% pivot_longer(N:Q99, names_to = "quartile")
